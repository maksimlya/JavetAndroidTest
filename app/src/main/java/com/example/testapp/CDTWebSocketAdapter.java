...
import com.example.testapp.staticModules.GsonWorker;
import com.example.testapp.staticModules.WebsocketText;
...
public class CDTWebSocketAdapter extends WebSocketAdapter implements IV8InspectorListener {
    ...
    public void onWebSocketText(String message) {
        ...
        try {
            WebsocketText parsed = (WebsocketText) GsonWorker.getInstance().fromGson(message, WebsocketText.class);
    ...
    public void runIfWaitingForDebugger(int contextGroupId) {
        try {
            CDTConfig.getV8Runtime().getExecutor(
                    "console.log('Welcome to Javet Android Debugging Environment!');").executeVoid();
        } catch (Exception e) {
            MainActivity.Log(e.getMessage());
        }
    }
